...
## Version
**Current Version:** 1.58.0 (Map in-map route editing, route filter dropdown, status-aware route display, routes page removed)
...
- **Map In-Map Route Editing** (`/tenant/map`): Edit existing routes directly on the map without leaving the page. Click the **Edit** button in a route's popup (gated by `tenant_routes.update`) to enter edit mode — the route's polyline is replaced with draggable circle markers (green = start, route color = middle, red = end). Drag any marker to reposition it; click the map to append a new point; tap × in the points list to remove a point. A floating edit panel (same style as draw panel) exposes Name, Type, Status, Parent Route, Color, Thickness, Description, and per-point widget selection. **Save Changes** calls `PUT /api/tenant/routes/:uuid`; Cancel exits without saving. Starting draw cancels any active edit and vice versa.
...
- **Route 360 Popup** (`/tenant/map`): Clicking a route polyline shows a rich detail card — color swatch, auto-generated code badge, status badge, type, thickness, point count, parent route, description, and created/updated dates — plus an **Edit** button for users with `tenant_routes.update`.
