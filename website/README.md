...
## Version
**Current Version:** 1.68.0 — Route Point Templates modal: 36 field flags + isDevice classification, searchable dropdowns with icon previews, Classification moved below Name, icon/device-type conditional visibility; SearchableSelect reusable component; duplicate permission groups fixed
...
- **Map In-Map Route Editing** (`/tenant/map`): Edit existing routes directly on the map without leaving the page. Hover a route line to open the info popup; click **Edit** (gated by `tenant_routes.update`) to enter edit mode — the polyline is replaced with draggable circle markers (green = start, route color = middle, red = end). Drag any marker to reposition it; click a **midpoint handle** (small hollow circle between points) to insert a new point between two existing ones; click the map to append at the end; tap × in the points list to remove a point. A floating edit panel exposes Name, Type, Status, Parent Route, Color, Thickness, Description, per-point Widget, Device Type, Name, and Description. An × button in the panel header closes the edit session. **Save Changes** calls `PUT /api/tenant/routes/:uuid`; Cancel exits without saving. Starting draw cancels any active edit and vice versa. Panel is mobile-responsive (bottom-anchored on small screens).
- **Route Filter Dropdown** (`/tenant/map`): The Routes section in the filter panel now shows a searchable dropdown listing all available routes. Selecting a specific route isolates it on the map; selecting "All routes" shows everything. The dropdown options respect the active Status filter. A visible match counter shows `n / total routes visible`.
- **Status-Aware Route Display** (`/tenant/map`): Routes are now loaded without a hard-coded `active` filter — all non-deleted routes are fetched. The **Status** filter (All / Active / Inactive / Maintenance) gates which routes appear on the map in real time. The "Maintenance" option was added to the status dropdown.
- **Route Line Hover Popup** (`/tenant/map`): Hovering over any route polyline opens a rich info card at the cursor — color swatch, code badge, status badge, type, thickness, point count, parent route, description, created/updated dates — plus an **Edit** button (RBAC-gated). The popup stays open for full interaction; close via × or click elsewhere.
- **Route Point Hover Tooltip** (`/tenant/map`): Hovering a route point marker shows a tooltip card with the point name, device type (blue), description, and point type/sequence. Points without an icon but with a name or description get a small grey dot marker for hover detection.
- **Route Point Name & Description** (`/tenant/map`): Each point in draw and edit panels has an optional **Point Name** text input; a **Description** field appears once a name is entered. Both fields are persisted to the backend and displayed in the hover tooltip.
- **Map Focus on Route Selection** (`/tenant/map`): Selecting a specific route from the Routes dropdown calls `map.flyToBounds()` to animate the map to fit the route (50 px padding, max zoom 17). GPS auto-recentering is paused while a route is focused; clearing the selection restores it.
- **Icons Management** (`/manage/icons`): Admin CRUD interface for reusable map icon/symbol assets (SVG, PNG, WebP). Codes are auto-generated in `ICOxxxx` format (e.g. `ICO0001`). Supports **file upload** — PNG/WebP via a click-to-upload drop zone with live preview; SVG via paste-in textarea or "Upload .svg file" link (FileReader fills the textarea client-side). Table view shows inline icon preview, type label, size, and status. Supports 10 icon types: Active Device, Passive Device, Power Device, Junction, Fiber Terminal, Splitter, Coupler, Route Point, Customer End, Flag. Always visible to all admin users (no extra permission needed).
